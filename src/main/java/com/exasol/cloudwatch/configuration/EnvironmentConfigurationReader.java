...
class EnvironmentConfigurationReader {
    ...
    EnvironmentConfigurationReader(final EnvironmentVariableProvider environmentVariableProvider) {
    ...
    String readDeploymentName() {
        final String name = Objects.requireNonNull(this.environmentVariableProvider.getEnv(EXASOL_DEPLOYMENT_NAME),
    ...
    List<String> readEnabledMetrics(final Collection<String> availableMetrics) {
        final String metricsParameter = this.environmentVariableProvider.getEnv("METRICS");
    ...
    String readExasolConnectionArn() {
        return Objects.requireNonNull(this.environmentVariableProvider.getEnv(EXASOL_CONNECTION_SECRET_ARN), ExaError
                .messageBuilder("E-CWA-15").message("EXASOL_CONNECTION_SECRET_ARN environment variable was not set.")
                .mitigation(
